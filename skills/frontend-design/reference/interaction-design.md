...
**Never `outline: none` without replacement.** It's an accessibility violation. Instead, use `:focus-visible` when available while keeping `:focus` as the fallback:
...
/* Hide the ring for pointer focus when :focus-visible is supported */
